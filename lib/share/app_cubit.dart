...
class AppCubit extends Cubit<AppStates> {
  ...
  TextEditingController textEmailController_loginScreen =
      TextEditingController();
  TextEditingController textPasswordController_loginScreen =
      TextEditingController();
  ...
  TextEditingController phoneNumberController_createOrderScreen =
      TextEditingController();
  TextEditingController streetAddressController_createOrderScreen =
      TextEditingController();
  TextEditingController cityController_createOrderScreen =
      TextEditingController();
  TextEditingController recieverNameController_createOrderScreen =
      TextEditingController();
  ...
  List<TextEditingController>
      attributeValueController_updateItemDetailsPostedByUser = [];
  ...
  TextEditingController nameController_updateItemDetailsPostedByUser =
      TextEditingController();
  TextEditingController descriptionController_updateItemDetailsPostedByUser =
      TextEditingController();
  TextEditingController priceController_updateItemDetailsPostedByUser =
      TextEditingController();
  ...
  TextEditingController phoneNumberController_userAddressScreen =
      TextEditingController();
  TextEditingController streetAddressController_userAddressScreen =
      TextEditingController();
  TextEditingController cityController_userAddressScreen =
      TextEditingController();
  TextEditingController recieverNameController_userAddressScreen =
      TextEditingController();
  ...
  void register_registerScreen(BuildContext context, String email,
      String password, String phone, String profileName) {
    emit(AppLoadingState());
    dio.post_registNewUser(email, password, phone, profileName).then((value) {
      if (value.statusCode == 400) {
        Fluttertoast.showToast(
            msg: value.data["errors"][0], toastLength: Toast.LENGTH_LONG);
      } else {
  ...
  void login_loginScreen(String email, String password, BuildContext context) {
    ...
    dio.post_login(email, password).then((value) {
      ...
      if (value.statusCode == 400) {
        Fluttertoast.showToast(
            msg: value.data["errors"], toastLength: Toast.LENGTH_LONG);
  ...
  void postNewCategory_addCategoryScreen(
      String name, String descreption, String imageUrl, BuildContext context) {
    emit(AppLoadingState());
    ...
    dio.post_category(name, descreption, imageUrl).then((value) {
      ...
      if (value.statusCode == 200) {
        ...
        Fluttertoast.showToast(
            msg: "Success Create Category", toastLength: Toast.LENGTH_LONG);
        ...
        for (int i = 0; i < categoryAttributes_addCategoryScreen.length; i++) {
          Dio()
              .post(post_AttributeType,
                  data: [
                    {
                      "categoryId": value.data["id"],
                      "attributeTypeName":
                          categoryAttributes_addCategoryScreen[i].toString()
                    }
                  ],
                  options: Options(
  ...
  void postNewItem_addItemScreen(BuildContext context, String name,
      String description, int price, String imageUrl, int categoryId) {
    ...
    Dio()
      ...
      Log.v("complete post item");
      {
        if (value.statusCode == 200) {
          ...
          getItemsData_itemModule();
          Fluttertoast.showToast(
              msg: "Success Create Item", toastLength: Toast.LENGTH_LONG);
  ...
  Future<Response> postImage(File image) async {
    ...
    return await Dio().post(post_Image,
        data: formData,
        options: Options(
    ...
    dio.get_attributeType(categoryId).then((value) {
      ...
      if (value.statusCode == 200) {
        ...
        for (int i = 0; i < value.data.length; i++) {
          attributeTypeByCategoryId_addItemScreen
              .add({"id": value.data[i]["id"], "name": value.data[i]["name"]});
          attributeValuesControllers_addItemScreen.add(TextEditingController());
        }
        Log.v(
            "Lenght of Controller ${attributeValuesControllers_addItemScreen.length}");
        Log.v(
            "Length of Attribute ${attributeTypeByCategoryId_addItemScreen.length}");
  ...
  Future postItemAttribute_addItemScreen(int itemId) async {
    ...
    for (int i = 0; i < attributeTypeByCategoryId_addItemScreen.length; i++) {
      await Dio()
          .post(post_AttributeValue,
              data: [
                {
                  "itemId": itemId,
                  "attributeTypeId": attributeTypeByCategoryId_addItemScreen[i]
                      ["id"],
                  "attributeValue": attributeValuesControllers_addItemScreen[i]
                      .text
                      .toString()
                }
              ],
  ...
  void getAttributeType_categoryDetails() {
    ...
    Dio()
        .get("$get_AttributeType${categorty_categoryDetails!.id}")
        .then((value) {
      if (value.statusCode == 200) {
  ...
  void getItemsPostedByUser() {
    ...
    dio.getItemsPostedByUser().then((value) {
      ...
      if (value.statusCode == 200) {
        ...
        for (int i = 0; i < value.data["data"].length; i++) {
          itemsPostedByUser_itemsByUser
              .add(ItemModel.fromJson(value.data["data"][i]));
        }
        emit(AppChangeState());
      } else {
  ...
  void deleteItemById_itemsByUser(int id) {
    ...
    dio.deleteItemById(id).then((value) {
      ...
      if (value.statusCode == 204 || value.statusCode == 200) {
        ...
        Fluttertoast.showToast(
            msg: value.data["error"], toastLength: Toast.LENGTH_LONG);
      } else {
  ...
  void searchItemCategoryByName_searchItemCategoryScreen(String name) {
    ...
    for (int i = 0; i < itemsByCategoryId.length; i++) {
      ...
      if (value.contains(name)) {
        searchItemsCategoryByName_searchItemsByCategoryScreen
            .add(itemsByCategoryId[i]);
      }
    }
    ...
    dio.getAttributeValues(id).then((value) {
      ...
      if (value.statusCode == 200) {
        ...
        for (int i = 0; i < value.data.length; i++) {
          ...
          attributeValuesItem_updateItemDetailsPostedByUser
              .add(AttributeValueModel.json(value.data[i]));
        }
  ...
  void updateItemDetails_updateItemsDetailsPostedByUser() {
    ...
    double price = double.parse(
        priceController_updateItemDetailsPostedByUser.text.toString());
    dio
        .putUpdateItemDetails(
      ...
      if (value.statusCode == 200) {
        ...
        for (int i = 0;
            i < attributeValuesItem_updateItemDetailsPostedByUser.length;
            i++) {
          Log.v("start Update Attribute value");
          dio
              .putUpdateAttributeValueForItem(
                  attributeValuesItem_updateItemDetailsPostedByUser[i].id,
                  attributeValueController_updateItemDetailsPostedByUser[i]
                      .text
                      .toString())
              .then((value2) {
  ...
  void postOrder_createOrderScreen(
    ...
    dio
        .postOrder(itemId, phoneNumber, streetAddress, city, recieverName)
        .then((value) {
      Log.v("Complete Post Order");
      if (value.statusCode == 200) {
        Log.v("Success Post Order");
        Fluttertoast.showToast(
            msg: "Success Order", toastLength: Toast.LENGTH_LONG);
        Navigator.pop(context);
        Navigator.pop(context);
      } else if (value.statusCode == 400) {
        Log.v("Success Post Order");
        Fluttertoast.showToast(
            msg: value.data.toString(), toastLength: Toast.LENGTH_LONG);
      } else {
  ...
  void getMyOrders_orderCreatedByLogedScreen() {
    ...
    dio.getBoughtOrder().then((value) {
      ...
      if (value.statusCode == 200) {
        ...
        for (int i = 0; i < value.data.length; i++) {
          myOrders_orderCreatedByLogedUserScreen
              .add(OrderModel.fromJson(value.data[i]));
        }
    ...
    dio.getItemDetails(itemId).then((value) {
      ...
      if (value.statusCode == 200) {
        ...
        for (int i = 0; i < value.data["orders"].length; i++) {
          requestedOrder_orderRequestedByOtherUserScreen
              .add(RequestedOrderModel.fromJson(value.data["orders"][i]));
        }
        emit(AppChangeState());
      } else {
  ...
  void rejectOrder_orderRequestedByOtherUser(int orderId, int itemId) {
    ...
    dio.putRejectOrder(orderId).then((value) {
      ...
      if (value.statusCode == 200) {
        ...
        Fluttertoast.showToast(
            msg: value.data.toString(), toastLength: Toast.LENGTH_SHORT);
  ...
  void getUserAddress_userAddressScreen() {
    dio.getUserAddress().then((value) {
      if (value.statusCode == 200) {
        ...
        phoneNumberController_userAddressScreen.text =
            userAddress_userAddressScreen!.phoneNumber;
        streetAddressController_userAddressScreen.text =
            userAddress_userAddressScreen!.streetAddress;
        cityController_userAddressScreen.text =
            userAddress_userAddressScreen!.city;
        recieverNameController_userAddressScreen.text =
            userAddress_userAddressScreen!.recieverName;
        emit(AppChangeState());
      } else {
  ...
  void postUserAddress_userAddressScreen(String phoneNumber,
      String streetAddress, String city, String recieverName) {
    dio
        .postUserAddress(phoneNumber, streetAddress, city, recieverName)
        .then((value) {
      if (value.statusCode == 200) {
