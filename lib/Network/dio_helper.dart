...
class DioHelper {
  ...
  Future<Response> post_registNewUser(
      String email, String password, String phone, String profileName) async {
    return await dio.post(post_RegisterUser, data: {
      "email": email,
      "password": password,
      "phone": phone,
  ...
  Future<Response> post_login(String email, String password) async {
    return await dio
        .post(post_LoginUser, data: {"email": email, "password": password});
  }
  ...
  Future<Response> getUserAddress() async {
    return await dio.get(get_UserAddress,
        options: Options(
            headers: {"Authorization": "bearer ${CacheHelper.getToken()}"}));
  }

  Future<Response> postUserAddress(String phoneNumber, String streetAddress,
      String city, String recieverName) async {
    return await dio.post(post_UserAddress,
        data: {
  ...
  Future<Response> getItemsPostedByUser() async {
    return await dio.get(get_GetItemsPostedByUser,
        options: Options(
            headers: {"Authorization": "bearer ${CacheHelper.getToken()}"}));
  }

  Future<Response> postOrder(int itemId, String phoneNumber,
      String streetAddress, String city, String recieverName) async {
    return await dio.post(post_CreateAnOrder,
        data: {
          "itemId": itemId,
          "phoneNumber": phoneNumber,
          "streetAddress": streetAddress,
  ...
  Future<Response> getBoughtOrders() async {
    return await dio.get(get_BoughtOrders,
        options: Options(
            headers: {"Authorization": "bearer ${CacheHelper.getToken()}"}));
  }

  Future<Response> putUpdateAttributeValueForItem(int id, String value) async {
    return await dio.put("$put_AttributeValue$id",
        data: {"id": id, "value": value},
        options: Options(
            headers: {"Authorization": "bearer ${CacheHelper.getToken()}"}));
  }

  Future<Response> putUpdateItemDetails(int id, String name, String discription,
      double price, int categoryId) async {
    return await dio.put("$put_UpdateAnItemById$id",
        data: {
          "name": name,
  ...
  Future<Response> deleteItemById(int id) async {
    return await dio.delete("$delete_DeleteAnItemById$id",
        options: Options(
            headers: {"Authorization": "bearer ${CacheHelper.getToken()}"}));
  }

  Future<Response> getSoldOrder() async {
    return await dio.get(get_SoldOrders,
        options: Options(
            headers: {"Authorization": "bearer ${CacheHelper.getToken()}"}));
  }
  ...
  Future<Response> putCanselOrder(int orderId) async {
    return await dio.put("$put_CanselOrder$orderId",
        options: Options(
            headers: {"Authorization": "bearer ${CacheHelper.getToken()}"}));
  }

  Future<Response> putRejectOrder(int orderId) async {
    return await dio.put("$put_RrjrctOrder$orderId",
        options: Options(
            headers: {"Authorization": "bearer ${CacheHelper.getToken()}"}));
  }

  Future<Response> getItemDetails(int itemId) async {
    return await dio.get("$get_ItemDetails$itemId",
        options: Options(
            headers: {"Authorization": "bearer ${CacheHelper.getToken()}"}));
  }

  Future<Response> startProcessingOrder(int orderId) async {
    return await dio.put("$put_StartProcessing$orderId",
        options: Options(
            headers: {"Authorization": "bearer ${CacheHelper.getToken()}"}));
  }

  Future<Response> confirmPayment(int orderId) async {
    return await dio.put("$put_ConfirmPayment$orderId",
        options: Options(
            headers: {"Authorization": "bearer ${CacheHelper.getToken()}"}));
  }

  Future<Response> shipOrder(int orderId) async {
    return await dio.put("$put_ShipOrder$orderId",
        options: Options(
            headers: {"Authorization": "bearer ${CacheHelper.getToken()}"}));
  }
}
